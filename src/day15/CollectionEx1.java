...
public class CollectionEx1 {
	...
	public static void main(String[] args) {
		/* Num class의 객체만 저장가능한 list를 ArrayList로 생성한다.
		 * 
		 */
		ArrayList<Num>list = new ArrayList<Num>();
		list.add(new Num());
		//Num클래스를 상속받은 자식클래스 Num2를 이용한 객체는 에러 발생하지 않음
		//자식 클래스를 부모 클래스로 자동 형변환할 수 있기 때문에
		//list.add(new Num2());
		//Ch클래스를 이용한 객체는 Num클래스를 이용한 객체가 아니므로 에러가 발생한다.
		//list.add(new Ch());
		list.add(new Num());
		list.add(new Num(1));
		...
		/////////////////////////////////
		while(it.hasNext()) {//hasNext() : 다음번지가 있는지 없는지 확인한다.
			System.out.print(it.next() + " ");
		}
		///////////////////////////////// ***Iterator를 쓸때는 while문 공식이다.
		System.out.println();
		/*
		 * while(it.hasNext()) {
		 *  	Num tmp = it.next();
			System.out.print(tmp + " ");   ******위의 while공식과 동일하다!!
		}
		 */
		list.ensureCapacity(100); //용량이 100개 되도록한다. 100짜리 배열을 만들어놓겠다는 의미 
		List<Num> list3 = list.subList(0,2); // subList는 부분리스트이다.
		System.out.println("0번지에서 2번지 사이의 부분리스트(2번지 앞까지 (2번지제외))");
		printList(list3);
		/*
		 * 익명클래스는 주로 메소드에서 인터페이스를 구현한 클래스가 필요한데, 
		 * 해당 메소드가 기능이 하나뿐이라서 따로 정의하기 귀찮은 경우 익명클래스를 이용한다.
		 */
		list.sort(new Comparator<Num>() {//익명클래스이다. Comparator : 비교문이다.
